...
export function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "hover:bg-accent/50 not-in-data-[variant=card]:data-selected:bg-accent/50 relative border-b",
        className,
      )}
      {...props}
    />
...
export function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "text-muted-foreground h-10 px-2.5 text-left align-middle leading-none font-medium whitespace-nowrap in-data-[variant=bordered]:border-e in-data-[variant=bordered]:last:border-e-0 has-[[role=checkbox]]:w-px",
        className,
      )}
      {...props}
    />
...
export function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn(
        "bg-clip-padding p-2.5 align-middle leading-none whitespace-nowrap in-data-[slot=table-footer]:py-3.5 in-data-[variant=bordered]:border-e in-data-[variant=bordered]:last:border-e-0 in-data-[variant=card]:first:ps-[calc(--spacing(2.5)-1px)] in-data-[variant=card]:last:pe-[calc(--spacing(2.5)-1px)] has-[[role=checkbox]]:w-px",
        className,
      )}
      {...props}
    />
